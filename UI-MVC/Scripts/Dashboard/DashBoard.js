...
>>>>>>> master
    ...
    addElement = function(grid) {
      ...
      if (ZoneId === 'x') {
        ...
        addZone = addZone.children('.add-zone').children('div').children('#zone-x');
        var plusElement = addZone.data('gridstack')
          .addWidget($(
              '<div><div class="grid-stack-item-content bgc-white bd" id ="Element-+"><div><img class="w-3r bdrs-50p alert-img add-element" src="/Content/Images/plus-icon.png"><div/><div/><div/>'),
            0,
            0,
            3,
            3,
        ...
        plusElement.children('.grid-stack-item-content').children('div').children('img').on("click",
          function() {
            addElement(addZone);
          });
      };

      var newElement = grid.data('gridstack')
        .addWidget($(
            '<div><div class="grid-stack-item-content bgc-white bd" id="Element-x"><i class="ti-trash float-right mR-15 mT-15 delete-element"></i><canvas></canvas><div/><div/>'),
          0,
          0,
          3,
          3,
      ...
      var Element = JSON.parse('{"X" : "' +
        ...
        '", "Y" : "' +
        Y +
        '", "Width": "3", "Height": "3", "IsDraggable": "true", "ZoneId": "' +
        ZoneId +
        '"}');
      ...
      plusElement = grid.data('gridstack')
        .addWidget($(
            '<div><div class="grid-stack-item-content bgc-white bd" id ="Element-+"><div><img class="w-3r bdrs-50p alert-img add-element" src="/Content/Images/plus-icon.png"><div/><div/><div/>'),
          0,
          0,
      ...
      newElement.children('#Element-' + ElementId).children('.delete-element').on('click',
        function() {
      ...
      $('.btn-next').on('click',
        function() {
          ...
          var cardnumber = 1;
          $('#locality-dropdown').on('change',
            function() {
              if (cardnumber <= 5) {
                $('#cardholder').append($('<div id="card' +
                  cardnumber +
                  '" class="col-sm-2-5 pX-5">' +
                  ...
                  '</div>' +
                  '</div>'));
                $('#card' + cardnumber).children('.card').children('img').on('click',
                  function() {
                    var item = ($.ajax({
                      async: false,
                      type: 'GET',
                      headers: Headers,
                      url: "https://localhost:44342/api/item/getitem/" +
                        $("#locality-dropdown option:selected").attr('value')
                    }).responseJSON);
      ...
      $('.btn-finish').on('click',
        function() {
          ...
          var Element = JSON.parse('{"ElementId":"' +
            ElementId +
            '", "X" : "' +
            newElement.data().gsX +
            '", "Y" : "' +
            newElement.data().gsY +
            '", "Width": "' +
            newElement.data().gsWidth +
            '", "Height": "' +
            newElement.data().gsHeight +
            '", "IsDraggable": "' +
            true +
            '", "ZoneId": "' +
            ZoneId +
...
<<<<<<< HEAD
      ...
      titleTag.replaceWith($('<input type="text" name="title" style="width: ' +
        titleWidth +
        'px; height:' +
        titleHeight +
...
>>>>>>> master
              ...
              grid.data('gridstack').addWidget(
                $('<div><div class="grid-stack-item-content bgc-white bd" id="Element-' +
                  ElementId +
                  '"><i class="ti-trash float-right mR-15 mT-15 delete-element"></i><canvas></canvas><div/><div/>'),
                Element.X,
                Element.Y,
                Element.Width,
                Element.Height,
    ...
    loadGrid = function(grid) {
      ...
      ZoneId = ZoneId.substring(ZoneId.indexOf('-') + 1, ZoneId.length);
      if (ZoneId === 'x') {
        var plusElement = grid.data('gridstack')
          .addWidget($(
              '<div><div class="grid-stack-item-content bgc-white bd" id ="Element-+"><div><img class="w-3r bdrs-50p alert-img add-element" src="/Content/Images/plus-icon.png"><div/><div/><div/>'),
            0,
            0,
        ...
        grid.each(function() {
          ...
          _.each(elements,
            function(node) {
              var newElement = griddata.addWidget(
                $('<div><div class="grid-stack-item-content bgc-white bd" id="Element-' +
                  node.ElementId +
                  '"><i class="ti-trash float-right mR-15 mT-15 delete-element"></i><div/><div/>'),
                node.X,
                node.Y,
                node.Width,
                node.Height);
              newElement.children('#Element-' + node.ElementId).children('.delete-element').on('click',
                function() {
                  deleteElement($(this), grid);
                });

              newElement.children('#Element-' + node.ElementId).append($('<canvas></canvas>')).children('canvas')
                .attr('id', 'pie-chart');
          ...
          var plusElement = griddata.addWidget(
            $(
              '<div><div class="grid-stack-item-content bgc-white bd" id ="Element-+"><div><img class="w-3r bdrs-50p alert-img add-element" src="/Content/Images/plus-icon.png"><div/><div/><div/>'),
            0,
            0,
            ...
            datasets: [
              {
                ...
                data: values,
                backgroundColor: [
                  "#ff6384", "#36a2eb", "#cc65fe", "#ffce56", "#7DDF64", "#7BDFF2", "#FDE74C", "#47E5BC", "#DAD6D6",
                  "#EF3E36"
                ]
              }
            ]
    ...
    $(".grid-stack").each(function() {
      ...
      $(this).parent().siblings('h4').children('.delete-zone').on('click',
        function() {
          removeZone($(this).parent().parent());
        });

      $(this).parent().siblings('h4').children('.edit-zone').on('click',
        function() {
          editZone($(this));
        });
