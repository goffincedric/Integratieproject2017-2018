...
namespace UI_CA_Prototype
{
    public class ExtensionMethods
    {
        ...
        {
            if (subplatforms.ToList().Count == 0)
                throw new Exception("Er zijn nog geen subplatforms, maak er eerst een aan");
            Console.WriteLine("\nSubplatforms: ");
            int counter = 1;
        ...
        {
            records.OrderBy(r => r.Date).ThenBy(r => r.Persons[0].Name).ThenBy(r => r.Tweet_Id).ToList()
        ...
        public void WriteTestRecords(IEnumerable<Record> records)
        {
            ...
            using (StreamWriter sw =
                new StreamWriter(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) +
                                 @"\structure.json"))
            ...
            Console.WriteLine(
                "Er werd een testbestand genaamd 'structure.json' weggeschreven naar uw desktop met max 200 test-records");
        }
    }
}